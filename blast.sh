...
function blast_genes () {
    for i in $(ls $2/housekeep/*.f*a* & ls  $2/phos/*.f*a); do
        ...
        blastn -query $i -db $1/*.f*a -outfmt 6 -evalue 0.00001 \
            -max_hsps 1 -max_target_seqs 1 -task "dc-megablast" | \
            sort -nrk 4,4  >> $DIR/blast/dcMega.out

        blastn -query $i -db $1/*.f*a -outfmt 6 -evalue 0.00001 \ 
            -max_hsps 1 -max_target_seqs 1 | \
            sort -nrk 4,4  >> $DIR/blast/blast.out
    done
}
